...
This makes a prediction but I don't understand how to compare:

```{r}
pred_author = predict(out1, newx = Xtest, s = "lambda.min")
pred_author = data.frame((pred_author))
```
...
```{r}
library(stringr)
pred_max = colnames(pred_author)[apply(pred_author,1,which.min)]
pred_max = str_sub(pred_max, end=-3)
```

```{r}
compare = cbind.data.frame(ytest, pred_max)
compare[] <- lapply(compare, as.character)
compare$comp = compare$ytest == compare$pred_max
```

```{r}
length(which(compare$comp))
```
